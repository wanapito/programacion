...
public class Baneuro {
    ...
    private static void nuevaCuenta() {
       ...
       while(!validarTipoCuenta){
           tipoCuenta = Teclado.leerTexto("Introduce 'c' para crear una cuenta corriente o 'a' para crear una cuenta de ahorro");
       ...
       //do{
        //   tipoCuenta = Teclado.leerTexto("Introduce 'c' para crear una cuenta corriente o 'a' para crear una cuenta de ahorro");
       //}while(tipoCuenta.equalsIgnoreCase("c") || tipoCuenta.equalsIgnoreCase("a"));
       ...
       if(tipoCuenta.equalsIgnoreCase("c")){
           cuentaCreada = new CuentaCorriente(new CCC(entidad, oficina), nombre, apellidos, saldo);
           arrayCuentas.add(cuentaCreada);
       }else if (tipoCuenta.equalsIgnoreCase("a")){
           cuentaCreada = new CuentaAhorro(new CCC(entidad, oficina), nombre, apellidos, saldo);
           arrayCuentas.add(cuentaCreada);
       }
    }
    ...
    private static void ingresar() {
        ...
        double ingreso = 0;
        numeroCuentaIngresado = Teclado.leerTexto("Introduce el numero de cuenta en el que realizar el ingreso");
        for (int i = 0; i < arrayCuentas.size(); i++) {
            System.out.println(arrayCuentas.get(i).getNumeroCuentaCCC().toStringCCCSinEspacios().equals(numeroCuentaIngresado));

            if(arrayCuentas.get(i).getNumeroCuentaCCC().toStringCCCSinEspacios().equals(numeroCuentaIngresado) ){
    ...
    private static void reintegro() {
        ...
        for (int i = 0; i < arrayCuentas.size(); i++) {
            if(arrayCuentas.get(i).getNumeroCuentaCCC().toStringCCCSinEspacios().equals(numeroCuentaIngresado)){
                System.out.println("Cuenta correcta");
                do{
                retirado = Teclado.leerDouble("Introduce el saldo a retirar");
                }while(retirado <= 0 || arrayCuentas.get(i).getSaldo() < retirado);
    ...
    private static void listar() {
        ...
        for (int i = 0; i < arrayCuentas.size(); i++) {
            listado += "Cuenta: " + i + "\n";
            listado += "CCC: " +  arrayCuentas.get(i).getNumeroCuentaCCC().toStringCCC() + "\n";
            ...
            System.out.println("CCC: " +  arrayCuentas.get(i).getNumeroCuentaCCC().toStringCCC());
            System.out.println("Nombre: " +  arrayCuentas.get(i).getNombre());
            System.out.println("Apellidos: " +  arrayCuentas.get(i).getApellidos());
            ...
            if(arrayCuentas.get(i) instanceof CuentaAhorro){
                ...
                System.out.println("Interes: " +  arrayCuentas.get(i).getInteres());
            }
        }
    }
    ...
    private static void salvarNumeroCuenta(){
        ...
        CCC.setContador(Integer.parseInt(arrayCuentas.get(arrayCuentas.size()-1).getNumeroCuentaCCC().getNumeroCuenta()));
    }

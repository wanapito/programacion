...
public class CCC implements Serializable {
	...
	public CCC(String entidad, String oficiona) {
		...
		this.digitosControl = obtenerDigitosControl(entidad, oficiona,
				numeroCuenta);
		this.codigoCCC = toStringCCC();
	}

	private String obtenerDigitosControl(String entidad, String oficina,
			String numeroCuenta) {
		String dControl;
		dControl = primerDigitoControl(entidad, oficina)
				+ segundoDigitoControl(numeroCuenta);
		return dControl;
	}
	...
	private String primerDigitoControl(String entidad, String oficina) {
		...
		for (int i = 0; i < primerDigitoControl.length; i++) {
			sumado += Integer.parseInt(entidadOficinaArray[i])
					* primerDigitoControl[i];
		}
	...
	private String segundoDigitoControl(String numeroCuenta) {
		...
		for (int i = 0; i < segundoDigitoControl.length; i++) {
			sumado += Integer.parseInt(numeroCuentaArray[i])
					* segundoDigitoControl[i];
		}
		numeroFinal = 11 - (sumado % 11);
		if (numeroFinal == 10) {
			digitoControl = String.valueOf(1);
	...
	String toStringCCC() {
		return entidad + " " + oficina + " " + digitosControl + " "
				+ numeroCuenta;
	}

...
public class Mensajeria {
    ...
    private static void leerFicheroBinario() {
        ...
        if(fichero.exists()){
            ...
            try{
                ...
                listaMensajes = (ArrayList<Mensaje>) objInputStream.readObject();
            }catch(IOException e){
    ...
    private static void recuperarContador(){
        if(!listaMensajes.isEmpty()){
            Mensaje.contador = listaMensajes.get(listaMensajes.size() - 1).getCodigoNumerico();
        }
    }
    ...
    private static void listarTodosMensajes() {
        ...
        boolean opcionElegidaValida = false;String mensaje;
        
        while(!opcionElegidaValida){
            opcionElegida = Teclado.leerTexto("¿Que quieres hacer? \n Listar: 'l' \nImprimir: 'i'");
            if(opcionElegida.equalsIgnoreCase("l") || opcionElegida.equalsIgnoreCase("i")){
    ...
    private static void listarNormales() {
        for (Mensaje mensaje : listaMensajes) {
            if(mensaje instanceof MensajeSinEncriptar ){
                System.out.println("***************\nCodigo: " + mensaje.getCodigoNumerico() + "\nMensaje: " + mensaje.getMensaje() + "\n***************");
            }
        }
    }
    ...
    private static void buscarMensajes() {
        ...
        for(int i = 0; i < listaMensajes.size(); i++){
            if(listaMensajes.get(i).getCodigoNumerico() == codigoBuscado){
                System.out.println("***************\nCodigo: " + listaMensajes.get(i).getCodigoNumerico() + "\nMensaje: " + listaMensajes.get(i).getMensaje() + "\n***************");
                break;
    ...
    private static void anadirMensajes() {
        ...
        while(!tipoMensajeValido){
            tipoMensaje = Teclado.leerTexto("¿Que tipo de mensaje quieres crear? \n Normal: 'n' \nEncriptado: 'e'");
            if(tipoMensaje.equalsIgnoreCase("n") || tipoMensaje.equalsIgnoreCase("e")){
    ...
    private static void desencriptarMensajes() {
        ...
        for(int i = 0; i < listaMensajes.size(); i++){
            if(listaMensajes.get(i).getCodigoNumerico() == codigoBuscado && listaMensajes.get(i) instanceof MensajeEncriptado){
                System.out.println("***************"
                        + "\nCodigo: " + listaMensajes.get(i).getCodigoNumerico() 
                        + "\nMensaje: " + listaMensajes.get(i).getMensaje() 
                        + "\nMensaje desencriptado: " + ((MensajeEncriptado)listaMensajes.get(i)).desEncriptar()
                        + "\n***************");
                break;
            }else if(i == listaMensajes.size() - 1){
                System.out.println("El codigo no existe o no es un mensaje encriptado.");
            }
        }
    }
    ...
    private static void eliminarMensajes() {
        ...
        if(codigoBorrrado){
            for(int j = indiceBorrado; j < listaMensajes.size(); j++){
                listaMensajes.get(j).setCodigoNumerico(listaMensajes.get(j).getCodigoNumerico() - 1);
            }
        }
    }
}

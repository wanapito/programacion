...
public class Taller {
	...
	private static void nuevoTrabajo() {
		...
		while (!respuestaValida) {
			switch (trabajoElegido) {
				case 1 :
					descripcion = Teclado.leerTexto(
							"Indica una descripcion para el trabajo.");
	...
	private static void anadirHoras() {
		...
		if (indiceEncontrado != -1) {
			numeroHoras = Teclado
					.leerEntero("Indica el numero de horas a añadir");
	...
	private static void anadirMaterial() {
		...
		if (indiceEncontrado != -1) {
			precioMaterial = Teclado
					.leerDouble("Indica el importe del material a añadir");
			if (trabajos.get(indiceEncontrado) instanceof RepMecanica) {
				((RepMecanica) trabajos.get(indiceEncontrado))
						.usarMaterial(precioMaterial);
			} else if (trabajos
					.get(indiceEncontrado) instanceof RepChapaPintura) {
				((RepChapaPintura) trabajos.get(indiceEncontrado))
						.usarMaterial(precioMaterial);
			}
	...
	private static void finalizarTrabajo() {
		...
		if (indiceEncontrado != -1) {
			...
			generarFicheroTexto(
					trabajos.get(indiceEncontrado).getIdentidicadorTrabajo(),
					trabajos.get(indiceEncontrado).toString());
		}
	}
	...
	private static int comprobarID(String id) {
		...
		for (int i = 0; i < trabajos.size(); i++) {
			if (trabajos.get(i).getIdentidicadorTrabajo().equals(id)
					&& !trabajos.get(i).isEstaFinalizado()) {
				return i;
			} else {
		...
		if (noEstaEnListaOEstaFinalizado) {
			System.out.println(
					"El identificador no esta en la lista o el trabajo esta finalizado.");
		}
		return -1;
	}

	private static void generarFicheroTexto(String rutaFichero,
			String informacionAEscribir) {
	...
	private static void recuperarUltimoIdentificador() {
		if (!trabajos.isEmpty()) {
			String ultimoIdentificador = trabajos.get(trabajos.size() - 1)
					.getIdentidicadorTrabajo();

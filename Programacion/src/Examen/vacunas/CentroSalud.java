...
public class CentroSalud {
	...
	private static void registrarPaciente() {
		// Registramos el paciente, pedimos todos los datos, hay 2 funciones al final para validar telefono y fecha
		...
		while(!formatoTelefonoCorrecto){
			numeroTelefono = Teclado.leerTexto("Indica el telefono del paciente");
		...
		while(!formatoFechaCorrecto){
			fechaPrimeraDosis = Teclado.leerTexto("Indica la fecha de la primera dosis (Formato dd/mm/aaaa)");
		...
		switch(opcionVacuna){
		  case 1:
			  ...
			  nuevaVacuna = new VacunaMonodosis(nombreVacuna, fechaPrimeraDosis);
		      nuevoPaciente = new Paciente(nombreApellidos, numeroTelefono, nuevaVacuna);
		      pacientes.add(nuevoPaciente);
			  break;
		  case 2:
			  // Vacuna multidosis
			// Si es multidosis instanciamos Vacuna como multidosis
			  int semanasParaSegundaVacuna = Teclado.leerEntero("Indica el numero de semanas para la siguiente dosis");
			  nuevaVacuna = new VacunaMultidosis(nombreVacuna, fechaPrimeraDosis, semanasParaSegundaVacuna);
		      nuevoPaciente = new Paciente(nombreApellidos, numeroTelefono, nuevaVacuna);
		      pacientes.add(nuevoPaciente);
		      break;
		}
	...
	private static void mostrarPacientesCompleta() {
		...
		for (Paciente valor : pacientes) {
			...
			}else if(valor.getVacuna() instanceof VacunaMultidosis && ((VacunaMultidosis)valor.getVacuna()).comprobarFechaSegundaDos()) {
				String vacunaMultidosis = valor.toString();
				vacunaMultidosis += "\n Tipo de vacuna: Vacuna Multidosis\n"
						+ "Fecha Segunda Dosis: " + ((VacunaMultidosis)valor.getVacuna()).getFechaSegundaVacunaString()
						+ "\n-------------------------------";
	...
	private static void mostrarPacientePendiente() {
		// Recorremos el Arraylist y mostramos los que les falta la segunda dosis
		for (Paciente valor : pacientes) {
			if(valor.getVacuna() instanceof VacunaMultidosis && !((VacunaMultidosis)valor.getVacuna()).comprobarFechaSegundaDos()) {
				String vacunaMultidosis = valor.toString();
				vacunaMultidosis += "\n Tipo de vacuna: Vacuna Multidosis\n"
						+ "Fecha Segunda Dosis: " + ((VacunaMultidosis)valor.getVacuna()).getFechaSegundaVacunaString()
						+ "\n-------------------------------";
